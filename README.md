...
`report_status_histories`는 상태 변경 전체 이력을 추적하기 위한 감사 로그로 사용하고, 화면 표시 및 외부 보고 양식에는 `repair_reports`에 저장된 주요 처리 시점 정보를 우선 사용한다.
...
PATCH  /api/reports/{reportId}/resubmit
POST   /api/reports/{reportId}/export
GET    /api/reports/{reportId}/histories
